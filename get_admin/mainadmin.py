...
token = "5628656983:AAHUzEE6nQjByDcb0TYmjtiBQwPoVTI_Jn4"
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                def word_filter(words:list,mem):
                    count = 0
                    for i in words:
                        i = i.strip()
                        i = i.lower()
                        if i != '' and i != ' ':
                            if i in mem.user.first_name.lower():
                                count += 1
                            elif i in mem.user.username.lower():
                                count += 1
                            elif mem.user.last_name :
                                if i in mem.user.last_name.lower():
                                    count += 1
                    if count > 0:
                        return True
                    return False
                ...
                for member in Cli.iter_chat_members(Chats_id):
                    if member.user.username != None and member.user.is_bot == False and word_filter(words,member) == True:
